...
There are different ways to load the client script. Some of the ways are:
...
If you call the `signIn` / `createAccount` functions before the client library is loaded, such calls will reject with the [`PLAY_SERVICES_NOT_AVAILABLE` error code](../errors).



To read the official docs for loading the Client Library, [click here](https://developers.google.com/identity/gsi/web/guides/client-library).
