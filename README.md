...
With the One Tap sign in, you can combine the one-tap methods with those one from the legacy sign in, i.e. you can use the One Tap sign in to sign in the user. Then call `signInSilently()` and then call `getCurrentUser()` to get the user data.
